...
cdef object p_var_backbone(int path_size, double p, object path_dist):
    # Input:
    # * path_size >= 0 integer
    # * p >= 1 real
    ...
    cdef object ret = collections.namedtuple("p_var", ["value", "points"])
    ...
    cdef int s = path_size - 1
    cdef int N = 1
    while s >> N != 0:
        N += 1

    cdef array.array ind = array.array('d', [0.0] * s)
    cdef double[:] cind = ind

    def ind_n(int j, int n):
        return (s >> n) + (j >> n)

    def ind_k(int j, int n):
        return min(((j >> n) << n) + (1 << (n - 1)), s)

    cdef double max_p_var = 0.0
    cdef array.array run_p_var = array.array('d', [0.0] * path_size)
    cdef double[:] crun_p_var = run_p_var

    cdef array.array point_links = array.array('i', [0] * path_size)
    cdef int[:] cpoint_links = point_links
        ...
        while True:
            ...
            if n > 0:
                ...
                if delta >= iid:
                    ...
                    delta = pow(max_p_var - run_p_var[m], 1.0 / p)
                    delta_m = m
                    if delta >= iid:
                        skip = True
...
def ex_sq():
    ...
    dist = lambda a, b: math.sqrt(
        pow(path[b][0] - path[a][0], 2) + pow(path[b][1] - path[a][1], 2)
    )
    ...
    while p <= 4.0:
        ...
        print(
            f"{p:5.2f}-variation: {pv.value:7.2f}, error {pv_err:.2e}, sequence"
            f" {pv.points}"
        )
        p += 0.5
...
def ex_bm():
    ...
    for p in [1.0, math.sqrt(2), 2.0, math.exp(1)]:
        ...
        pv_err = abs(pv.value - pv_ref) + p_var_points_check(pv, p, dist)
        print(
            f"{p:5.2f}-variation: {pv.value:7.2f}, sequence length:"
            f" {len(pv.points):5d}, error {pv_err:.2e}, time: {pv_time:7.2f}, reference"
            f" time: {pv_ref_time:7.2f}"
        )
...
def ex_bm_long():
    ...
    for n in [0, 1, 10, 100, 1000, 10000, 100000, 1000000, 10000000]:
        ...
        pv_time = time.time() - pv_start
        print(
            f"{n:10d} steps: {p:5.2f}-variation: {pv.value:7.2f}, sequence length:"
            f" {len(pv.points):5d}, time: {pv_time:7.2f}"
        )
